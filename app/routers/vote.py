...
def vote(vote:schemas.Vote,db: Session = Depends(database.get_db),current_user:int =Depends(oauth2.get_current_user)):
    ...
    vote_query = db.query(models.Voter).filter(models.Voter.post_id== vote.post_id, models.Voter.user_id == current_user.id)
    ...
    if (vote.dir == 1):
        ...
        new_vote = models.Voter(post_id =vote.post_id,user_id= current_user.id)
        db.add(new_vote)
        db.commit()
